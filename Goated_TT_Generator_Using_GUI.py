...
class SemesterGUI:
    ...
    def schedule_practical(self, subject, teacher, timetable, teacher_schedules):
        days = list(timetable.keys())
        random.shuffle(days)

        for day in days:
            # Find practical slots (2 consecutive slots)
            for i in range(len(time_slots) - 1):
                slot1 = time_slots[i]
                slot2 = time_slots[i + 1]

                # Combine slots to create a 2-class duration
                combined_slot = f"{slot1.split('-')[0]}-{slot2.split('-')[1]}"

                # Ensure practical classes do not start at restricted times
                if any(restricted in slot1 for restricted in ['9:55', '12:00']):
                    continue

                # Check availability for combined slots
                if (self.is_slot_available(day, slot1, teacher, timetable, teacher_schedules) and
                    ...
                    # Determine batch subjects for overlapping scheduling
                    batch_subjects = self.get_batch_subjects(subject)

                    if batch_subjects:
                        combined_labels = []

                        for batch_num in range(1, 4):  # Ensure all 3 batches get overlapping labs
                            overlapping_labs = "/".join(
                                [f"{lab_subject} (Lab - Batch {batch_num})" for lab_subject in batch_subjects]
                            )
                            combined_labels.append(overlapping_labs)
